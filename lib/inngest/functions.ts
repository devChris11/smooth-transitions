...
export const processEnrollment = inngest.createFunction(
  ...
  async ({ event, step }) => {
    try {
      ...
      const brief = await step.run(
        "generate-context",
        async (ctx: FetchContextResult): Promise<string> => {
...
Student: ${ctx.student.name} (${ctx.student.student_number})
Course: ${ctx.course.name} (${ctx.course.course_code})
Credits: ${ctx.course.credits ?? "N/A"}
Category: ${ctx.course.category ?? "N/A"}
Completion method: ${ctx.course.completion_method ?? "N/A"}
Description: ${ctx.course.description ?? "N/A"}
...
Be direct and specific. No filler. Write in second person addressing the student directly.`;
      ...
      await step.run(
        "save-context",
        async (generatedBrief: string): Promise<{ success: boolean }> => {
          const supabase = createServiceClient();
